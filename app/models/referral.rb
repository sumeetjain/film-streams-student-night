class Referral < ActiveRecord::Base
	...
	def self.get_referrals_by_event(event_id)
		@get_referrals_from_event = ActiveRecord::Base.connection.execute("SELECT COUNT(*) AS total, referral_type FROM attendances JOIN referrals ON attendances.student_id = referrals.student_id WHERE attendances.event_id = #{event_id} GROUP BY referral_type ORDER BY total DESC;")
	    @referrals = {}
